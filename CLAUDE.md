...
- **풀블리드 캔버스(0.1.3):** `#canvas-wrap`은 `position:fixed; inset:0; z-index:0`으로 **창 전체**에 깔리고, 상단바·사이드바는 그 위(`z-index:10`)에 뜬다. 덕분에 일반 창/오버레이의 **스테이지 좌표공간이 0~화면폭으로 동일** → 오버레이 진입 시 이미지가 안 움직이고 오른쪽 빈 띠 없음. (이전엔 사이드바 132px만큼 캔버스가 좁아 오버레이에서 우측이 비었음.) 단, 좌표 원점이 화면 좌상단(0,0)이라 일반 창에선 상단바/사이드바 뒤로 이미지가 가려질 수 있음.
- **사이드바 접기(0.1.3):** 타이틀바 `≡`(`#btn-sidebar-toggle`) → `body.sidebar-collapsed` 토글, `settings.json`에 기억. 규칙은 `:not(.overlay-mode)`로 스코프.
- **떠있는 도구막대 드래그(0.1.3):** 오버레이 편집모드에서 `#sidebar-grip`을 포인터 드래그 → `#sidebar` 인라인 `left/top`. **편집모드 벗어나면 인라인 해제**(창모드 `#sidebar`는 `position:relative`라 인라인 좌표 남으면 레이아웃 깨짐). 위치는 세션 내 `floatPos`로 기억.
- 함정(다음 작업 시 주의):
